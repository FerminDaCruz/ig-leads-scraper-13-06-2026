...
function startOfDayUTC(ar: Date): Date {
  return new Date(
    Date.UTC(ar.getUTCFullYear(), ar.getUTCMonth(), ar.getUTCDate()) + AR_MS,
  );
}
...
function getPeriod(
  period: string,
  dateParam?: string,
): { from: Date; to: Date; label: string; exactDay: boolean } {
  const ar = arNow();
  const now = new Date();
  switch (period) {
    case "hoy":
    ...
    case "dia": {
      if (dateParam) {
        ...
        const label = arD.toLocaleDateString("es-AR", {
          weekday: "long",
          day: "2-digit",
          month: "2-digit",
          year: "numeric",
    ...
    case "semana": {
      ...
      const d = new Date(ar);
      d.setUTCDate(ar.getUTCDate() - (dow === 0 ? 6 : dow - 1));
      return {
        from: startOfDayUTC(d),
        to: now,
...
function avgDays(pairs: { a: string | null; b: string | null }[]): string {
  const diffs = pairs
    .filter((p) => p.a && p.b)
    .map(
      (p) => (new Date(p.b!).getTime() - new Date(p.a!).getTime()) / 86400000,
    );
...
export default async function StatsPage({
  ...
  const [
    ...
    supabase
      ...
      .gte("first_seen_at", fromStr)
      .lt("first_seen_at", toStr),
    supabase
      .from("leads")
      .select("id", { count: "exact", head: true })
      .gte("qualified_at", fromStr)
      .lt("qualified_at", toStr)
      .eq("calificado", true),
    supabase
      .from("leads")
      .select("id", { count: "exact", head: true })
    ...
    supabase
      .from("leads")
      .select("id", { count: "exact", head: true })
      .gte("contacted_at", fromStr)
      .lt("contacted_at", toStr),
    supabase
      .from("searches")
      .select("*")
      .gte("ran_at", fromStr)
    ...
    supabase
      .from("leads")
      .select("first_seen_at, qualified_at")
      .gte("qualified_at", fromStr)
      .lt("qualified_at", toStr)
      .not("calificado", "is", null),
    supabase
      .from("leads")
      .select("qualified_at, contacted_at")
      .gte("contacted_at", fromStr)
      .lt("contacted_at", toStr),
    supabase
      .from("leads")
      .select("descarte_razon")
      .gte("qualified_at", fromStr)
      .lt("qualified_at", toStr)
      .eq("calificado", false),
  ]);
  ...
  const reviewTime = avgDays(
    (reviewTimeRes.data || []).map((r) => ({
      a: r.first_seen_at,
      b: r.qualified_at,
    })),
  );
  const contactTime = avgDays(
    (contactTimeRes.data || []).map((r) => ({
      a: r.qualified_at,
      b: r.contacted_at,
    })),
  );

  const reasonMap = (reasonsRes.data || []).reduce(
    (acc, r) => {
  ...
  const reasonEntries = Object.entries(reasonMap)
    .map(([k, n]) => ({
      label: RAZON_LABELS[k] || k,
      count: n,
      pct:
        disqualifiedCount > 0 ? Math.round((n / disqualifiedCount) * 100) : 0,
    }))
  ...
  const avgNewLeads =
    searches.length > 0
      ? Math.round(
          (searches.reduce((s, r) => s + r.new_leads, 0) / searches.length) *
            10,
        ) / 10
      : 0;
...
`;
  ...
  const summaryCards = [
    {
      label: "Encontrados",
      value: foundCount,
      sub: `${searches.length} búsqueda${searches.length !== 1 ? "s" : ""}`,
      color: "text-brand",
    },
    {
      label: "Revisados",
      value: reviewedCount,
      sub: pct(reviewedCount, foundCount) + " de encontrados",
      color: "text-navy dark:text-cream",
    },
    {
      label: "Calificados",
      value: qualifiedCount,
      sub: pct(qualifiedCount, reviewedCount) + " de revisados",
      color: "text-green-600 dark:text-green-400",
    },
    {
      label: "Descartados",
      value: disqualifiedCount,
      sub: pct(disqualifiedCount, reviewedCount) + " de revisados",
      color: "text-red-500 dark:text-red-400",
    },
    {
      label: "Contactados",
      value: contactedCount,
      sub: pct(contactedCount, qualifiedCount) + " de calificados",
      color: "text-navy dark:text-cream",
    },
  ];
  ...
  const funnelRows = [
    ...
    },
    {
      ...
      n: reviewedCount,
      chainBase: foundCount,
      chainLabel: "de encontrados",
      color: "bg-brand/70",
    },
    {
      label: "Calificados",
      n: qualifiedCount,
      chainBase: reviewedCount,
      chainLabel: "de revisados",
      color: "bg-green-500",
    },
    {
      label: "Descartados",
    ...
    },
    {
      label: "Contactados",
      n: contactedCount,
      chainBase: qualifiedCount,
      chainLabel: "de calificados",
      color: "bg-green-700",
    },
  ];
    ...
    <main className="max-w-7xl mx-auto px-6 py-6">
      ...
      <div className="grid lg:grid-cols-5 gap-4 mb-4">
        ...
        <Card className="lg:col-span-3">
          <CardHeader className="pb-2">
            ...
            <CardDescription>
              Barras y columna derecha: % de encontrados ({foundCount}) ·
              columna izquierda: % del paso anterior
            </CardDescription>
          </CardHeader>
          <CardContent className="p-5 pt-2 flex flex-col gap-4">
            {funnelRows.map((row) => {
              const pctChain =
                row.chainBase > 0
                  ? Math.round((row.n / row.chainBase) * 100)
                  : 0;
              const pctOfFound =
                foundCount > 0 ? Math.round((row.n / foundCount) * 100) : 0;
              return (
                <div key={row.label} className="flex flex-col gap-1.5">
                  ...
                  <div className="w-full bg-surface dark:bg-navy rounded-full h-2.5 overflow-hidden">
                    <div
                      className={`h-full rounded-full ${row.color} transition-all`}
                      style={{ width: `${Math.max(pctOfFound, 1)}%` }}
                    />
                  </div>
                </div>
              );
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-surface dark:bg-navy rounded-xl p-3">
                ...
                <p className="text-2xl font-bold text-green-600 dark:text-green-400">
                  {pct(qualifiedCount, reviewedCount)}
                </p>
              </div>
              <div className="bg-surface dark:bg-navy rounded-xl p-3">
                <p className="text-xs text-muted mb-1">Tasa descarte</p>
                <p className="text-2xl font-bold text-red-500 dark:text-red-400">
                  {pct(disqualifiedCount, reviewedCount)}
                </p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
                ...
                <div className="flex flex-col gap-2">
                  {reasonEntries.map((r) => (
                    <div key={r.label} className="flex items-center gap-2">
                      <span className="w-32 text-xs text-navy dark:text-cream/80 truncate shrink-0">
                        {r.label}
                      </span>
      ...
      <div className="grid lg:grid-cols-5 gap-4 mb-4">
          ...
          <CardContent className="p-4 pt-2 grid grid-cols-2 gap-3">
            <div className="bg-surface dark:bg-navy rounded-xl p-3">
              ...
              <p className="text-2xl font-bold text-navy dark:text-cream">
                {pct(contactedCount, qualifiedCount)}
              </p>
        ...
        <Card className="lg:col-span-3">
          <CardHeader className="pb-2">
            ...
            <CardDescription>
              Actividad del scraper en el período
            </CardDescription>
          </CardHeader>
      ...
      <Card className="overflow-x-auto">
        <CardHeader>
          ...
          <CardDescription>
            Ordenadas por leads nuevos · {searches.length} en total
          </CardDescription>
        </CardHeader>
        <Table>
          <TableHeader>
          ...
          <TableBody>
            {searches.length === 0 ? (
              <TableRow>
                <TableCell colSpan={5} className="text-center text-muted py-10">
              ...
              searches.map((s, i) => (
                <TableRow key={i}>
                  <TableCell className="text-navy dark:text-cream/80">
                    {s.niche}
                  </TableCell>
                  <TableCell className="text-navy dark:text-cream/80">
                    {s.location}
                  ...
                  <TableCell className="text-center">
                    <Badge variant={s.new_leads > 0 ? "success" : "secondary"}>
                      {s.new_leads}
                    </Badge>
                  </TableCell>
